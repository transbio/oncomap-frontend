var map_container, resources;
var backend_url = 'http://oncoapi.biotektools.org/api';
window.onload = init();
...
function init() {
    ...
    resources = new DumpsterLayer(backend_url + '/resources/tiles/{z}/{x}/{y}/?format=json');
    //dumpsters_clustered = new ClusteredDumpsterLayer('api/dumpster/tiles/clustered/{z}/{x}/{y}');
    resources.addToMap(map_container.map, 0, 21);
...
function Map() {
    ...
    this.bind_to_ui = function (div_id) {
        // Karte laden
        this.map = L.mapbox.map(div_id).setView([43.296418, -0.368205], 7);
